...
class FacebookNLLBTranslator(ITranslator):
    ...
    async def translate_batch(
        ...
        try:
            ...
            to_lang_code = "vie_Latn"

            # ---------------------------------------------------------
            # Gộp toàn bộ text thành 1 chuỗi
            # ---------------------------------------------------------
            ...
            #
            # Vì toàn bộ text được gửi thành 1 lần nên source language
            # cũng phải dùng chung.
            # ---------------------------------------------------------
            src_lang = self._resolve_src_lang(from_lang, merged_text)

            self.tokenizer.src_lang = src_lang

            # ---------------------------------------------------------
            # Tokenize toàn bộ chuỗi
            # ---------------------------------------------------------
            ...
            logger.info(
                f"NLLB: translating {len(texts)} texts as ONE input "
                f"[{src_lang}→{to_lang_code}]"
            )
            ...
            # ---------------------------------------------------------
            # Tách lại thành từng câu
            # ---------------------------------------------------------
            translated_texts = translated.splitlines()

            # Đảm bảo số lượng output khớp input
            if len(translated_texts) != len(texts):
                logger.warning(
                    f"NLLB output count mismatch: "
                    f"input={len(texts)}, output={len(translated_texts)}"
                )

                # Fallback nếu model làm thay đổi xuống dòng
                translated_texts = [
                    x.strip()
                    for x in translated.split("\r\n")
                ]

            logger.info(
                f"NLLB Translate: {len(texts)} texts in ONE call, "
