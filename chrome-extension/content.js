...
(function () {
  ...
  if (!chrome || !chrome.storage) {
    console.error('[Content] Chrome API not available');
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    try {
      ...
      if (msg.type === 'START_SELECTION') {
        ...
        if (!document.body) {
          console.error('[Content] Document body not available');
          sendResponse({ status: 'error', message: 'Document not ready' });
          return true;
        }
        startSelection();
      ...
      if (msg.type === 'CAPTURE_SAVED') {
        ...
        if (!savedRect) {
          console.error('[Content] No region saved');
      ...
      console.error('[Content] Message handler error:', err);
  ...
  function startSelection() {
    try {
      ...
      console.error('[Content] startSelection error:', err);
      alert('Lỗi khi bắt đầu chọn vùng: ' + err.message);
    }
  }
  ...
  function capture(rect) {
    try {
      chrome.runtime.sendMessage({ type: 'capture' }, (res) => {
        ...
        img.onerror = () => {
          alert('Không thể load ảnh từ dataUrl');
        };
      });
    } catch (err) {
      console.error('[Content] capture error:', err);
      alert('Lỗi khi capture: ' + err.message);
    }
  }
})();
